...
/**
 * 使用dom技术对 xml文件进行crud操作<br>
 * dom解析的特点是会将xml文件一次加载到内存中，不适合大文件操作<br>
 * 大型xml可用sax解析，但是sax只支持解析，不支持增改删<br>
 */
public class XMLDom {
	...
	public static void main(String[] args) throws Exception {
		...
		// 指定解析哪个xml文件
		Document document = dBuilder.parse("src/main/resources/files/xml/classes.xml");
		// list(document);
		// read(document);
		// add(document);
		upd(document);
	...
	public static void upd(Document doc) throws Exception {
		...
		node_name.setTextContent("修改为宋江");
		...
		tf.transform(new DOMSource(doc), new StreamResult("src/main/resources/files/xml/classes.xml"));
	}
	...
	public static void list(Node node) {
		// xml的换行会显示为#text，对应的种类是node.TEXT_NODE
