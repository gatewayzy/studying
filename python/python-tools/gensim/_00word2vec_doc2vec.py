...
def word2vec(corpus_path=None, size=50):
    ...
    print('词语相似度：', dis)
    print(savedModel.most_similar("狐臭"))
    print(savedModel.most_similar("膝盖"))

    # 保存 wordvec文件
    f = open('./data/wv.txt', 'w+', encoding='utf-8')
    print(savedModel.wv)
    for i, word in enumerate(savedModel.wv.vocab):
        print(word)
        f.write(word + ' ' + ' '.join(map(str, savedModel[word])) + '\n')
